...
def run_comprehensive_benchmark(api_key, num_repeats=5):
    ...
    for mode in modes:
        ...
        for i, data in enumerate(tqdm(test_dataset)):
            ...
            try:
                ...
                if i % 5 == 0:
                    print(
                        f" [Step {i}] Rew: {reward:.2f} | Sav: {saving:.1%} | Valid: {res['valid']}"
                    )
    ...
    report_path = (
        f"results/final_report_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    )
...
if __name__ == "__main__":
    ...
    if not key:
        ...
        print(
            "Usage: export GEMINI_API_KEY='your_key' && python scripts/comprehensive_benchmark.py"
        )
    else:
