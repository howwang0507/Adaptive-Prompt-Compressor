...
class AsyncLinUCBCompressor:
    ...
    def __init__(self, api_key: str = None, model_name: str = "gemini-1.5-flash", 
                 alpha: float = 1.0, fallback_threshold: float = 0.6):
    ...
    async def execute_with_fallback(self, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await asyncio.to_thread(self.env.execute_request, prompt, meta["arm"])
            ...
            if result.get("semantic_score", 1.0) < self.fallback_threshold:
                print(f"⚠️ Quality below threshold ({result['semantic_score']}). Falling back...")
                result = await asyncio.to_thread(self.env.execute_request, prompt, arm=0) # Arm 0 is raw/conservative
