...
- 🔍 **Evidence-Preserving RAG Engine**: [`src/rag/evidence_compressor.py`](src/rag/evidence_compressor.py) (Query-evidence ranking & CJK/Chinese sentence segmentation)
- 💰 **End-to-End Net Benefit Model**: [`src/economics/economic_model.py`](src/economics/economic_model.py) (True ROI accounting for latency, CPU cost, and retry risk)
- 🛡️ **Block-Level Structural & Constraint Guard**: [`src/guards/structural_guard.py`](src/guards/structural_guard.py) (Code indentation & negation lock with auto safe fallback)
